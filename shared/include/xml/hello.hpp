...
namespace netd::shared::xml {
  ...
  public:
    ...
    static std::unique_ptr<Hello> fromXml(const std::string &xml,
  ...
  public:
    ...
    static std::unique_ptr<HelloToClient>
    toXml(uint32_t sessionId, const std::vector<std::string> &capabilities,
          const struct ly_ctx *ctx);
  ...
  public:
    ...
    static std::unique_ptr<HelloToServer> fromXml(const std::string &xml,
                                                  const struct ly_ctx *ctx);

    // Generate HelloToServer from parameters
    static std::unique_ptr<HelloToServer> toXml(lyd_node *lyd_data,
                                                const struct ly_ctx *ctx);
    static std::unique_ptr<HelloToServer>
    toXml(uint32_t sessionId, const std::vector<std::string> &capabilities,
          const struct ly_ctx *ctx);
