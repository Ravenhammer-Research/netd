...
namespace netd::shared::request {
  ...
  public:
    ...
    HelloRequest(netd::shared::netconf::NetconfSession *session,
                 struct lyd_node *rpc)
    ...
    static std::unique_ptr<HelloRequest> fromYang(const ly_ctx *ctx,
                                                  const lyd_node *node);
    static std::unique_ptr<HelloRequest>
    fromRpcEnvelope(const ly_ctx *ctx,
                    std::shared_ptr<netd::shared::xml::RpcEnvelope> envelope);
    static std::unique_ptr<HelloRequest>
    fromHelloToServer(const netd::shared::xml::HelloToServer &hello_to_server);
