...
namespace netd::shared::request {
  ...
  lyd_node *CommitRequest::toYang(ly_ctx *ctx) const {
    ...
    if (lyd_new_inner(nullptr, mod, "commit", 0, &commitNode) != LY_SUCCESS) {
      throw netd::shared::ArgumentError("toYang: failed to create commit element");
    }

    // Add confirmed element if present
    if (confirmed_) {
      lyd_node *confirmedNode = nullptr;
      if (lyd_new_inner(commitNode, mod, "confirmed", 0, &confirmedNode) != LY_SUCCESS) {
        lyd_free_tree(commitNode);
        throw netd::shared::ArgumentError("toYang: failed to create confirmed element");
      }

      // Add timeout if specified
      if (timeout_ > 0) {
        lyd_node *timeoutNode = nullptr;
        if (lyd_new_term(confirmedNode, mod, "confirm-timeout", std::to_string(timeout_).c_str(), 0, &timeoutNode) != LY_SUCCESS) {
          lyd_free_tree(commitNode);
          throw netd::shared::ArgumentError("toYang: failed to create confirm-timeout element");
        }
      }
    }
    ...
    if (!persist_.empty()) {
      ...
      if (lyd_new_term(commitNode, mod, "persist", persist_.c_str(), 0, &persistNode) != LY_SUCCESS) {
        lyd_free_tree(commitNode);
        throw netd::shared::ArgumentError("toYang: failed to create persist element");
      }
    }
  ...
  CommitRequest::fromYang([[maybe_unused]] const ly_ctx *ctx,
                          const lyd_node *node) {
    if (!node) {
      throw netd::shared::ArgumentError("Invalid YANG node provided to CommitRequest::fromYang");
    }
    ...
    while (commitNode && strcmp(lyd_node_schema(commitNode)->name, "commit") != 0) {
    ...
    if (commitNode) {
      ...
      while (confirmedNode && strcmp(lyd_node_schema(confirmedNode)->name, "confirmed") != 0) {
      ...
      if (confirmedNode) {
        ...
        while (timeoutNode && strcmp(lyd_node_schema(timeoutNode)->name, "confirm-timeout") != 0) {
      ...
      while (persistNode && strcmp(lyd_node_schema(persistNode)->name, "persist") != 0) {
