...
struct lyd_node *create_route_set_response(struct ly_ctx *ctx, const char *route_dest, bool success) {
    ...
    if (success) {
        ...
        if (ret == LY_SUCCESS) {
            ...
            snprintf(error_msg, sizeof(error_msg), "Route configuration failed for destination: %s", route_dest);
